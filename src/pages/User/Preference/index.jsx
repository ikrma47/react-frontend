...
import { getApplicationStatusAction, updateApplicationStatusAction } from "pages/App/ducks/actions";
...
const Preference = ({
	...
	const handleDeleteClick = async (delPreference) => {
		...
		try {
			...
			if (delPreference == "1st") updateApplicationStatusAction({ isPreference: false });
	...
	const onSubmit = async (event) => {
		...
		try {
			...
			await getCourseAction(appId);
			if (values.preference == "1st") updateApplicationStatusAction({ isPreference: true });
		...
		return (
			<Form noValidate onSubmit={onSubmit} validated={validated}>
				<Form.Row>
					...
					<Form.Control.Feedback type="invalid">cannot set preferences</Form.Control.Feedback>
					<Form.Group as={Col} controlId="departmentName">
						...
						>
							...
							{department?.data?.map(function renderSelectDepartmentMenu({ id, departmentName }) {
								return (
						...
						<Form.Control.Feedback type="invalid">select the department</Form.Control.Feedback>
					</Form.Group>
					<Form.Group as={Col} controlId="courseName">
						...
						<Form.Control.Feedback type="invalid">select the course</Form.Control.Feedback>
					</Form.Group>
				</Form.Row>
